...
**框选不是负担，不必每句都框。** 只框两种句子：**没听懂的**，以及**听懂了但你觉得重要、有价值的**。在备注里写下你认为最关键的信息——一个词或几个词都行。

于是 label 成了你对这段音频掌握情况的**真实反馈**：

- **label 少** —— 这段对你不难，可视为已掌握。
- **label 多** —— 恰恰说明这里有你的短板与提升空间。

换句话说，label 体现的是你的真实水平，而不是一项要做满的任务。初次精听完，如果你不想再一句句挨着听原音频，就把这些 label 导出，进入 **② 精听复习** 反复攻克。

> 边界始终由你的耳朵决定：纯手动框选，程序不做任何自动断句，Whisper 只为你已经标好的片段提供原文参照。这不是最自动化的做法，但它让标注忠实反映你的掌握度——这是我们想到的最好的精听方法。

- 波形拖拽框选，选中即自动回环
- 每段可写备注，记下最关键的信息
- 随手微调边界
- 导出自带 Whisper 原文的 ZIP 错题包，直通精听复习
